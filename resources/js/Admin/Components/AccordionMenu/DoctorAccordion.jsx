...
export default function DoctorAccordion({ addMenu, selectedLang }) {
    const [isOpen, setIsOpen] = useState(false)
    const [isSelectAll, setIsSelectAll] = useState(false)
    const [selectedItems, setSelectedItems] = useState([])
    const { doctors_menu: doctors } = usePage().props
    const doctors_menu = { data: doctors?.data[selectedLang] }

    // handle selected doctors menu
    ...
    const handleSelectAll = () => {
        if (isSelectAll) {
            ...
            setSelectedItems(doctors_menu.data)
            setIsSelectAll(true)
        }
    }
    ...
    useEffect(() => {
        ...
        setIsSelectAll(selectedItems.length === doctors_menu.data.length)
    }, [selectedItems, doctors_menu.data])
        ...
        <li className={`list-group-item ${isOpen ? "show" : ""}`}>
            ...
            <div className="list-group-content">
                <div className="list-group-content-item">
                    {doctors_menu.data.map((menu, index) => (
                        <div key={index} className="form-check">
                            <input
                                ...
                                id={`Doctors-${index}`}
                            />
                            <label className="form-check-label" htmlFor={`Doctors-${index}`}>
                                {menu.name}
                            </label>
                        </div>
                    ))}
                ...
                <div className="list-group-content-bottom mt-3">
                    <div className="form-check">
                        <input
                            ...
                            className="form-check-input"
                            id="doctors-select-all"
                        />
                        <label className="form-check-label" htmlFor="doctors-select-all">
                            Select All
                        </label>
                    </div>
