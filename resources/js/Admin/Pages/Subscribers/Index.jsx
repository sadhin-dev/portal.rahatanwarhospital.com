...
import { cloudDownloadOutline, mailOpenOutline, search } from "ionicons/icons"
...
export default function Index({ subscribers, sort }) {
    ...
    const handleMark = (subscriberId) => {
        ...
        if (existsMark) {
            const removeItem = markItems.filter((item) => item !== subscriberId)
    ...
    const handleBulkAction = () => {
        ...
        if (selectedOption === "Delete") {
            confirmMessage = `${translate("You want to delete selected subscribers")}?`
            ...
            <AdminLayouts>
                ...
                <div className="container-fluid">
                    <div className="yoo-uikits-heading">
                        <h2 className="yoo-uikits-title">{translate("All subscribers")}</h2>
                    </div>
                                ...
                                <div className="yooDataTableWrap">
                                    <div className="dataTables_heading">
                                        <div className="dataTables_heading_left">
                                            ...
                                            <a href={route("admin.subscribers.export")} className="btn btn-success btn-sm" download>
                                                <IonIcon
                                    ...
                                    <div id="yooDataTable_wrapper" className="dataTables_wrapper no-footer">
                                        <table id="yooDataTable" className="display dataTable no-footer" style={{ width: "100%" }}>
                                            <thead>
                                                <tr role="row">
                                                    ...
                                                    <ThSortable width="40%" sort={sort} onSorted={() => getResults(searchQuery)} column="email">
                                                        {translate("Email")}
                                                    </ThSortable>
                                            ...
                                            <tbody>
                                                {subscribers.data.map((subscriber, index) => (
                                                    <tr className="odd" key={index}>
                                                        <td className="sorting_1" onClick={() => handleMark(subscriber.id)}>
                                                            <div
                                                                className={`yoo-check-mark ${
                                                                    markItems.some((item) => item === subscriber.id) && "active"
                                                                }`}
                                                            />
                                                        </td>
                                                        <td>{subscriber.email}</td>
                                                        <td>{moment(subscriber.created_at).format("ll")}</td>
                                                        <td>
                                                            <div
                                                                className="d-flex"
                                                                style={{
                                                            ...
                                                            >
                                                                {hasPermission("subscribers.delete") && (
                                                                    <DeleteButton href={route("admin.subscribers.destroy", subscriber)} />
                                                                )}
                                                            </div>
                                                        </td>
                                                    </tr>
                                        ...
                                        {!subscribers.data.length && (
                                            ...
                                            >
                                                <p>{translate("No subscribers found")}!</p>
                                            </div>
                                        )}
                                        <div className="clear" />
                                    </div>
